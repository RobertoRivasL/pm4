...
public class PaginaRegistro extends PaginaBase implements IPaginaRegistro {
    ...
    public boolean esPaginaVisible() {
        try {
            return esperarElementoVisible(formularioRegistro, 10) && 
                   esperarElementoVisible(campoNombre, 5) &&
                   esperarElementoVisible(campoEmail, 5) &&
    ...
    public boolean verificarRegistroExitoso() {
        try {
            return esElementoVisible(mensajeExito) || !esPaginaVisible();
    ...
    public boolean validarElementosPagina() {
        try {
            boolean todosPresentos = esElementoVisible(campoNombre) &&
    ...
    private void limpiarCamposManualmente() {
        try {
            if (esElementoVisible(campoNombre)) campoNombre.clear();
            if (esElementoVisible(campoApellido)) campoApellido.clear();
            if (esElementoVisible(campoEmail)) campoEmail.clear();
            if (esElementoVisible(campoPassword)) campoPassword.clear();
            if (esElementoVisible(campoConfirmarPassword)) campoConfirmarPassword.clear();
            if (esElementoVisible(campoTelefono)) campoTelefono.clear();
            if (esElementoVisible(campoFechaNacimiento)) campoFechaNacimiento.clear();
        } catch (Exception e) {
            logger.debug(TipoMensaje.DEBUG.formatearMensaje(
