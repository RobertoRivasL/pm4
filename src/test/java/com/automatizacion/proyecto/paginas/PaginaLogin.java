...
public class PaginaLogin extends PaginaBase implements IPaginaLogin {
    ...
    public boolean esPaginaVisible() {
        try {
            return esperarElementoVisible(formularioLogin, 10) && 
    ...
    public boolean verificarLoginExitoso() {
        try {
            // Verificar redirección o elemento que indique éxito
            return !esPaginaVisible() || esElementoVisible(mensajeExito);
    ...
    public boolean validarElementosPagina() {
        try {
            boolean todosPresentos = esElementoVisible(campoEmail) &&
