...
public class PruebasRegistro extends BaseTest {
    ...
    public void testRegistroExitoso() {
        ...
        ModeloDatosPrueba datos = ModeloDatosPrueba.builder()
                ...
                .nombre("Roberto Rivas")
                .email("roberto.test." + System.currentTimeMillis() + "@example.com")
        ...
        capturarPantalla("inicio_" + datos.getCasoPrueba());

        boolean registroExitoso = paginaRegistro.registrarUsuario(datos);

        // Captura final del resultado
        capturarPantalla("resultado_" + datos.getCasoPrueba());
        ...
        if (!registroExitoso) {
            ...
            logValidacion("DEBUG - URL actual: " + urlActual);
            logValidacion("DEBUG - Título actual: " + titulo);

            // Verificar si hay mensajes de error
            String mensajeError = paginaRegistro.obtenerMensajeError();
            if (!mensajeError.isEmpty()) {
                logValidacion("DEBUG - Mensaje de error encontrado: " + mensajeError);
            }
        }

        Assert.assertTrue(registroExitoso,
                String.format("El registro debería ser exitoso con datos válidos. " +
                        "Datos: %s, URL actual: %s",
                        datos.getCasoPrueba(),
    ...
    @Test(priority = 2, description = "Verificar validación de email inválido", groups = { "regression", "registro",
            "negativo", "validacion" })
    ...
    @Test(priority = 3, description = "Verificar validación de contraseñas que no coinciden", groups = { "regression",
    ...
    @Test(priority = 4, description = "Verificar validación de campos obligatorios vacíos", groups = { "regression",
            "registro", "negativo", "validacion" })
    ...
    @Test(priority = 5, description = "Verificar elementos de la página de registro", groups = { "smoke", "registro",
            "ui" })
    ...
    @Test(priority = 6, dataProvider = "datosRegistroValidos", dataProviderClass = ProveedorDatos.class, description = "Verificar registro con múltiples datos válidos", groups = {
            "regression", "registro", "positivo", "datadriven" })
    ...
    @Test(priority = 7, dataProvider = "datosRegistroInvalidos", dataProviderClass = ProveedorDatos.class, description = "Verificar validación con múltiples datos inválidos", groups = {
            "regression", "registro", "negativo", "datadriven" })
